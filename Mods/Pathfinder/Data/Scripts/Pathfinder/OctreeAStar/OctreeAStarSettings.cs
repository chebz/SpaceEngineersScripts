...
namespace Pathfinder.OctreeAStar
{
    ...
    public class OctreeAStarSettings
    {
        ...
        private const bool DEFAULT_RENDER_OCTANTS = true;
        private const bool DEFAULT_RENDER_PATH = true;
        ...
        public void Load()
        {
            try
            {
                ...
                if (settingsData.OpenSetting == null) { OpenSetting = CloneSetting(DEFAULT_OPEN_SETTING); wroteDefaults = true; } else { OpenSetting = settingsData.OpenSetting; }
                if (settingsData.ClosedSetting == null) { ClosedSetting = CloneSetting(DEFAULT_CLOSED_SETTING); wroteDefaults = true; } else { ClosedSetting = settingsData.ClosedSetting; }
                if (settingsData.OpenReverseSetting == null) { OpenReverseSetting = CloneSetting(DEFAULT_OPEN_REVERSE_SETTING); wroteDefaults = true; } else { OpenReverseSetting = settingsData.OpenReverseSetting; }
                if (settingsData.ClosedReverseSetting == null) { ClosedReverseSetting = CloneSetting(DEFAULT_CLOSED_REVERSE_SETTING); wroteDefaults = true; } else { ClosedReverseSetting = settingsData.ClosedReverseSetting; }
                if (settingsData.NonLeafSetting == null) { NonLeafSetting = CloneSetting(DEFAULT_NON_LEAF_SETTING); wroteDefaults = true; } else { NonLeafSetting = settingsData.NonLeafSetting; }
                if (settingsData.UnexploredSetting == null) { UnexploredSetting = CloneSetting(DEFAULT_UNEXPLORED_SETTING); wroteDefaults = true; } else { UnexploredSetting = settingsData.UnexploredSetting; }
                if (settingsData.OccupiedSetting == null) { OccupiedSetting = CloneSetting(DEFAULT_OCCUPIED_SETTING); wroteDefaults = true; } else { OccupiedSetting = settingsData.OccupiedSetting; }
                if (settingsData.MeetSetting == null) { MeetSetting = CloneSetting(DEFAULT_MEET_SETTING); wroteDefaults = true; } else { MeetSetting = settingsData.MeetSetting; }

                MaxNodesPerFrame = settingsData.MaxNodesPerFrame > 0 ? settingsData.MaxNodesPerFrame : DEFAULT_MAX_NODES_PER_FRAME;
                if (settingsData.MaxNodesPerFrame <= 0) wroteDefaults = true;
